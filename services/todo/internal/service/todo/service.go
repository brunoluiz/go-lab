...
func (s *Service) CreateTask(ctx context.Context, req dto.CreateTaskRequest) (dto.CreateTaskResponse, error) {
	...
	id, err := uuid.NewV7()
	if err != nil {
		return dto.CreateTaskResponse{}, fmt.Errorf("failed to generate UUID: %w", err)
	}
	task := model.Task{
		ID:          id.String(),
		Title:       req.Title,
	...
	if err != nil {
		return dto.CreateTaskResponse{}, fmt.Errorf("failed to create task: %w", err)
	}
...
func (s *Service) GetTask(ctx context.Context, req dto.GetTaskRequest) (dto.GetTaskResponse, error) {
	...
	if err != nil {
		...
		return dto.GetTaskResponse{}, fmt.Errorf("failed to get task: %w", err)
	}
...
func (s *Service) ListTasks(ctx context.Context, _ dto.ListTasksRequest) (dto.ListTasksResponse, error) {
	...
	if err != nil {
		return dto.ListTasksResponse{}, fmt.Errorf("failed to list tasks: %w", err)
	}
...
func (s *Service) UpdateTask(ctx context.Context, req dto.UpdateTaskRequest) (dto.UpdateTaskResponse, error) {
	...
	if err != nil {
		...
		return dto.UpdateTaskResponse{}, fmt.Errorf("failed to update task: %w", err)
	}
...
func (s *Service) DeleteTask(ctx context.Context, req dto.DeleteTaskRequest) (dto.DeleteTaskResponse, error) {
	...
	if err != nil {
		...
		return dto.DeleteTaskResponse{}, fmt.Errorf("failed to delete task: %w", err)
	}
